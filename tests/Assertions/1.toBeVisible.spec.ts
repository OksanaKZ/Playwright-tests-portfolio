...
test.describe('Тестирование видимости элементов с toBeVisible()', () => {
  ...
  test('Тест элементов с разными типами скрытия', async ({ page }) => {
    ...
    const toggleDisplayElement = page.locator('#toggle-display');
    const toggleVisibilityElement = page.locator('#toggle-visibility');
    const toggleOpacityElement = page.locator('#toggle-opacity');

    await expect(toggleDisplayElement).not.toBeVisible();
    await expect(toggleVisibilityElement).not.toBeVisible();
    await expect(toggleOpacityElement).toBeVisible();
  });
